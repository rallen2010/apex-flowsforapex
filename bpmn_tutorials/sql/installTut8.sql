...
apex_string.join_clob(
  ...
  , q'[]'
  , q'[To prevent 2 users in the same lane working on the same task by accident, you can make a task reservation.  In Flow Monitor, try clicking the Padlock icon to reserve a task, and again to release it if you change your mind &amp; want someone else to do it!</bpmn:text>]'
